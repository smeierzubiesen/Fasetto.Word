...
namespace Fasetto.Word
{
    ...
    public class WindowResizer
    {
        ...
        #endregion Windows Message Pump
        ...
        private void WmGetMinMaxInfo(System.IntPtr hwnd, System.IntPtr lParam)
        {
            ...
            // NOTE: rcMonitor is the monitor size rcWork is the available screen size (so the area
            //       inside the taskbar start menu for example)

            // Size size limits (used by Windows when maximized) relative to 0,0 being the current
            // screens top-left corner
            //
            // - Position
            ...
            // BUG:
            // NOTE: I've noticed a bug which I think is Windows itself If your non-primary monitor
            //       has a greater width than your primary (or possibly due to the screen ratio's
            //       being different) then setting the max X on the monitor to the correct value
            //       causes it to scale wrong.
            //
            // The fix seems to be to set the max width only (height is fine) to that of the primary
            // monitor, not the current monitor
            //
            // However, 1 pixel different and the size goes totally wrong again so the fix doesn't
            // work when the taskbar is on the left or right
